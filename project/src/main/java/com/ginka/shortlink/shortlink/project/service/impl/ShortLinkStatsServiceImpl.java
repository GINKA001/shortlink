...
import com.ginka.shortlink.shortlink.project.dto.req.*;
...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public ShortLinkStatsRespDTO oneShortLinkStats(ShortLinkStatsReqDTO requestParam) {
        ...
        LinkAccessStatsDO pvUvUipStatusByShortLink = linkAccessLogsMapper.findPvUvUipStatusByShortLink(requestParam);
        // 基础访问详情
        ...
        return ShortLinkStatsRespDTO.builder()
                .pv(pvUvUipStatusByShortLink.getPv())
                .uv(pvUvUipStatusByShortLink.getUv())
                .uip(pvUvUipStatusByShortLink.getUip())
                .daily(daily)
    ...
    public IPage<ShortLinkStatsAccessRecordRespDTO> oneShortLinkStatsAccessRecord(ShortLinkStatsAccessRecordReqDTO requestParam) {
        LambdaQueryWrapper<LinkAccessLogsDO> lambdaQueryWrapper = Wrappers.lambdaQuery(LinkAccessLogsDO.class)
                .eq(LinkAccessLogsDO::getFullShortUrl, requestParam.getFullShortUrl())
                .eq(LinkAccessLogsDO::getGid, requestParam.getGid())
                .between(LinkAccessLogsDO::getCreateTime, requestParam.getStartDate(), requestParam.getEndDate())
                .eq(LinkAccessLogsDO::getDelFlag, 0)
                .orderByDesc(LinkAccessLogsDO::getCreateTime);
        IPage<LinkAccessLogsDO> shortLinkStatsAccessRecordLogPage = linkAccessLogsMapper.selectPage(requestParam, lambdaQueryWrapper);
        List<String> list = shortLinkStatsAccessRecordLogPage.getRecords().stream().map(LinkAccessLogsDO::getUser).toList();//获取用户名
        IPage<ShortLinkStatsAccessRecordRespDTO> convert = shortLinkStatsAccessRecordLogPage.convert(each -> BeanUtil.toBean(each, ShortLinkStatsAccessRecordRespDTO.class));
        ShortLinkStatusChongGouReqDTO bean = BeanUtil.toBean(requestParam, ShortLinkStatusChongGouReqDTO.class);
        List<Map<String,Object>> UvTypeList =linkAccessLogsMapper.selectUvTypeByUsers(bean,list);
        convert.getRecords().stream().forEach(
                each -> {
    ...
    public ShortLinkStatsRespDTO groupShortLinkStats(ShortLinkGroupStatsReqDTO requestParam) {
        List<LinkAccessStatsDO> listStatsByShortLink = linkAccessStatsMapper.listStatsByGroup(requestParam);//查询返回请求体中  有效的（被查询过的）需要进行查询到短链接信息 gid and url
        if (CollUtil.isEmpty(listStatsByShortLink)) {
            return null;
        }
        LinkAccessStatsDO pvUvUipStatusByShortLink = linkAccessLogsMapper.findPvUvUipStatusByGroup(requestParam);
        // 基础访问详情
        List<ShortLinkStatsAccessDailyRespDTO> daily = new ArrayList<>();//有效链接集合
        List<String> rangeDates = DateUtil.rangeToList(DateUtil.parse(requestParam.getStartDate()), DateUtil.parse(requestParam.getEndDate()), DateField.DAY_OF_MONTH).stream()
                .map(DateUtil::formatDate)//将日期转化为指定的json 字符串格式
        ...
        // 地区访问详情（仅国内）
        List<ShortLinkStatsLocaleCNRespDTO> localeCnStats = new ArrayList<>();
        List<LinkLocaleStatsDO> listedLocaleByShortLink = linkLocaleStatsMapper.listLocaleByGroup(requestParam);//按省份分类 生成对应的实体
        int localeCnSum = listedLocaleByShortLink.stream()
                .mapToInt(LinkLocaleStatsDO::getCnt)
                .sum();//获取总访问量
        listedLocaleByShortLink.forEach(each -> {
        ...
        // 小时访问详情
        List<Integer> hourStats = new ArrayList<>();//将每一小时的数据整合
        List<LinkAccessStatsDO> listHourStatsByShortLink = linkAccessStatsMapper.listHourStatsByGroup(requestParam);//每小时的访问量 集合
        for (int i = 0; i < 24; i++) {
            AtomicInteger hour = new AtomicInteger(i);
            int hourCnt = listHourStatsByShortLink.stream()
                    .filter(each -> Objects.equals(each.getHour(), hour.get()))
    ...
    public IPage<ShortLinkStatsAccessRecordRespDTO> shortLinkGroupStatsAccessRecord(ShortLinkGroupStatsAccessRecordReqDTO requestParam) {
        ...
        IPage<ShortLinkStatsAccessRecordRespDTO> convert = shortLinkStatsAccessRecordLogPage.convert(each -> BeanUtil.toBean(each, ShortLinkStatsAccessRecordRespDTO.class));
        ShortLinkStatusChongGouReqDTO bean = BeanUtil.toBean(requestParam, ShortLinkStatusChongGouReqDTO.class);
        List<Map<String,Object>> UvTypeList =linkAccessLogsMapper.selectGroupUvTypeByUsers(bean,list);
        convert.getRecords().stream().forEach(
                each -> {
                    String uvType = UvTypeList.stream()
                            .filter(item -> Objects.equals(each.getUser(), item.get("user")))
